...
class Adapter_Dept(val context: Context,val deptList:ArrayList<Department_Data>) : RecyclerView.Adapter<Adapter_Dept.DeptViewHolder>(){



    class DeptViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {

        var imageDept=itemView.findViewById<ImageView>(R.id.img_dept)
        var txtDeptName=itemView.findViewById<TextView>(R.id.txt_deptName)

        var clickDept=itemView.findViewById<LinearLayout>(R.id.click_dept)

    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DeptViewHolder {

        val itemView= LayoutInflater.from(parent.context).inflate(R.layout.departments_shape1,parent,false)
        return Adapter_Dept.DeptViewHolder(itemView)
    }
    ...
    override fun onBindViewHolder(holder: DeptViewHolder, position: Int) {
        ...
        holder.clickDept.setOnClickListener {
